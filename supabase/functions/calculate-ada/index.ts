...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // =======================================================================
        // THE NAN-KILLER: Safely parses configuration variables that might be
        // set to boolean `true` in the matrix override system.
        // =======================================================================
        const num = (val: any, fallback: string) => {
        ...
        const matDict: any = {
            '1/32': { cost: num(config.Cost_Sub_Tactile, "55"), yield: sheet1152, name: '1/32" Tactile', costKey: 'Cost_Sub_Tactile' },
            '1/16': { cost: num(config.Cost_Sub_ADA_Core_116, "50"), yield: sheet1152, name: '1/16" Core', costKey: 'Cost_Sub_ADA_Core_116' },
            '1/8': { cost: num(config.Cost_Sub_ADA_Core_18, "70"), yield: sheet1152, name: '1/8" Core', costKey: 'Cost_Sub_ADA_Core_18' },
            '3mm': { cost: num(config.Cost_Sub_PVC, "29.09"), yield: sheet4608, name: '3mm PVC Backer', costKey: 'Cost_Sub_PVC' },
            '6mm': { cost: num(config.Cost_Stock_6mm_4x8, "58.37"), yield: sheet4608, name: '6mm PVC Backer', costKey: 'Cost_Stock_6mm_4x8' },
            '3/16': { cost: num(config.Cost_Sub_Acrylic, "91.65"), yield: sheet4608, name: '3/16" Clear Acrylic', costKey: 'Cost_Sub_Acrylic' },
            '1/32_CLR': { cost: num(config.ADA_APP_132_CLR, "58.12"), yield: sheet1152, name: '1/32" Clear Lens', costKey: 'ADA_APP_132_CLR' }
        };
        ...
        L(`File Preflight`, (num(config.Time_Preflight_Job, "15") / 60) * rateOp, `${V('Time_Preflight_Job')} Mins * ${V('Rate_Operator')}`);
        L(`Engraver Handling/Load`, (num(config.Time_Engraver_Load_Per_Item, "2") * inputs.qty / 60) * rateOp, `Qty * ${V('Time_Engraver_Load_Per_Item')} Mins * ${V('Rate_Operator')}`);
        L(`Engraver Machine Run`, ((totalSqin * num(config.Time_Engrave_SqIn, "0.25")) / 60) * engraveRate, `Total SqIn * ${V('Time_Engrave_SqIn')} Mins * ${V('Rate_Machine_Engraver')}`);

        let totalBeads = tactileLayers > 0 ? (inputs.qty * 10) : 0;
        if (totalBeads > 0) {
            L(`Braille Beads`, totalBeads * num(config.Cost_Raster_Bead, "0.01") * wastePct, `10 Beads/Sign * ${V('Cost_Raster_Bead')} * ${V('Waste_Factor')}`);
        ...
        if (hasEngravedWindow) {
            ...
            L(`Slider Paint Fill Material`, sliderSqin * num(config.Cost_Paint_SqIn, "0.01") * wastePct, `Slider SqIn * ${V('Cost_Paint_SqIn')} * ${V('Waste_Factor')}`);
        }

        if (hasCNC) {
            L(`CNC Router Run`, ((totalSqin * num(config.Time_CNC_Easy_SqFt, "1") / 144) / 60) * cncRate, `(Total SqFt * ${V('Time_CNC_Easy_SqFt')}) Mins * ${V('Rate_Machine_CNC')}`);
            L(`CNC Operator`, ((totalSqin * num(config.Time_CNC_Easy_SqFt, "1") / 144) / 60) * rateOp, `(Total SqFt * ${V('Time_CNC_Easy_SqFt')}) Mins * ${V('Rate_Operator')}`);
        }
        ...
        let baseRetailSqIn = num(config.Retail_Price_ADA_Basic_AB, "1.60");
        if (inputs.product === 'BasicClear') baseRetailSqIn = num(config.Retail_Price_ADA_Basic_Clear, "1.80");
        ...
        if (hasPaperWindow || hasEngravedWindow) {
            let hasBacker = safeLayers.some((l: any) => l.type === '3mm' || l.type === '6mm' || l.type === '3/16');
